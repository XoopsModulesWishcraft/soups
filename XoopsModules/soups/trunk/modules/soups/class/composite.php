 ...
 * @package kernel
 */
class SoupsComposite extends XoopsObject
{
	var $language = '';
	
    function SoupsComposite($id = null)
    {
...
* @package kernel
*/
class SoupsCompositeHandler extends XoopsPersistableObjectHandler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, "composite", 'SoupsComposite', "id", "symbol");
    }
	...
	function get($id = 0) {
		...
		if ($id<>0) {
			$obj = parent::get($id);
			$criteria = new CriteriaCompo(new Criteria('composite_id', $id));
	...
	function getObjects($criteria = NULL, $id_as_key = false, $as_object = true) {
		...
		foreach($objs as $id => $obj) {
			$criteria = new CriteriaCompo(new Criteria('composite_id', $obj->getVar('id')));
			$langobjs = $languageHandler->getObjects($criteria, false);			if (is_object($langobjs[0])) {
	...
	function insert($obj, $force = true, $setlanguage) {
		...
		if ($id = parent::insert($obj, $force)) {
			$obj->language->setVar('composite_id', $id);
